...
import { XMarkIcon, CalendarIcon, UserIcon, FlagIcon, ClockIcon, CheckIcon, ArrowRightIcon, CurrencyDollarIcon, ClipboardDocumentListIcon, BoltIcon, PlusIcon, SparklesIcon, ChatBubbleLeftRightIcon } from '../../constants';
...
const TriggerConfigModal: React.FC<TriggerConfigModalProps> = ({
  ...
  const handleCrimsonGPTPrompt = async () => {
    ...
    setTimeout(() => {
      const parsedLogic = parseNaturalLanguageLogic(crimsonGPTPrompt);

      if (parsedLogic) {
        // Add the parsed condition and actions to the form
        const newCondition = {
          id: Date.now().toString(),
          conditions: parsedLogic.conditions,
          actions: parsedLogic.actions
        };
  ...
  // Parse natural language into IF/THEN logic
  const parseNaturalLanguageLogic = (query: string) => {
    ...
    if (lowerQuery.includes('amount') && lowerQuery.includes('greater than')) {
      const amountMatch = lowerQuery.match(/greater than \$?(\d+)/);
      if (amountMatch) {
        conditions.push({
    ...
    if (lowerQuery.includes('create') && lowerQuery.includes('task')) {
      actions.push({
        id: Date.now().toString() + '_action_1',
        type: 'add-task',
        config: {
          taskType: 'todo',
          title: 'Follow-up Task',
          description: 'Generated by CrimsonGPT',
          priority: 'medium'
        }
      });
    }
    ...
    if (lowerQuery.includes('send') && lowerQuery.includes('email')) {
      actions.push({
        id: Date.now().toString() + '_action_2',
        type: 'send-to-mailchimp',
        config: {
          listName: 'Follow-up Campaign'
        }
      });
    }

    if (lowerQuery.includes('apply') && lowerQuery.includes('tag')) {
      actions.push({
        id: Date.now().toString() + '_action_3',
        type: 'apply-smart-tag',
        config: {
          smartTagId: '1',
          smartTagName: 'Needs Follow-up'
        }
      });
    }
    ...
    if (actions.length === 0) {
      actions.push({
        id: Date.now().toString() + '_default_action',
        type: 'add-task',
        config: {
          taskType: 'todo',
          title: 'Follow-up Required',
          description: 'Generated by CrimsonGPT based on: ' + query
        }
      });
    }
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-200px)]">
          ...
          {activeTab === 'conditions' && (
            <div className="space-y-6">
              ...
              {/* CrimsonGPT Natural Language Logic Builder */}
              <div className="bg-gradient-to-r from-crimson-blue to-crimson-dark-blue rounded-lg p-4 shadow-lg">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h4 className="font-medium text-white">CrimsonGPT Logic Builder</h4>
                ...
                <div className="space-y-2">
                  <div className="flex gap-2">
                    <input
                      ...
                      placeholder="Describe your logic: 'donors with past due tasks should have a new follow-up task created'"
                    ...
                    <button
                      ...
                      className="px-4 py-2 bg-white text-crimson-blue rounded-lg hover:bg-gray-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed font-medium flex items-center gap-1.5 text-sm"
                    >
                      <SparklesIcon className="w-3.5 h-3.5" />
                      {isProcessingPrompt ? 'Processing...' : 'Generate'}
                    </button>
                  </div>

                  <p className="text-white text-xs opacity-90">
                    CrimsonGPT will create IF/THEN rules automatically from your description.
                  </p>

                  <div className="flex flex-wrap gap-1.5 mt-2">
                    <button
                      onClick={() => setCrimsonGPTPrompt("donors with past due tasks should have a new task created for follow-up")}
                      className="text-xs bg-white bg-opacity-20 text-white px-2 py-1 rounded-full hover:bg-opacity-30 transition-colors"
                    >
                      "Past due tasks → Create follow-up"
                    </button>
                    <button
                      onClick={() => setCrimsonGPTPrompt("high priority tasks should send email notification")}
                      className="text-xs bg-white bg-opacity-20 text-white px-2 py-1 rounded-full hover:bg-opacity-30 transition-colors"
                    >
                      "High priority → Email alert"
                    </button>
                    <button
                      onClick={() => setCrimsonGPTPrompt("gifts over $500 should apply big giver tag")}
                      className="text-xs bg-white bg-opacity-20 text-white px-2 py-1 rounded-full hover:bg-opacity-30 transition-colors"
                    >
                      "Large gifts → Apply tag"
                    </button>
                  </div>
                </div>
              </div>
